...
@section('title', 'Hasil Prediksi Produksi Kelapa')

@section('header', 'Hasil Perhitungan Prediksi Produksi Kelapa')
...
<div class="container">
    ...
    @if(Auth::user()->role == 'admin')
        ...
        <form action="{{ route('kelapa.store') }}" method="POST">
            @csrf
    ...
    <div class="mb-3">
        <h4>Unduh Data</h4>
        <a href="{{ route('kelapa.export.pdf') }}" class="btn btn-danger">Download PDF</a>
        <a href="{{ route('kelapa.export.csv') }}" class="btn btn-success">Download CSV</a>
    </div>
                    ...
                    <table class="table table-sm table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if(Auth::user()->role == 'admin')
                                <td>
                                    <form action="{{ route('kelapa.destroy', $prediction['periode']) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                    </form>
                                    <a href="{{ route('kelapa.edit', $prediction['periode']) }}" class="btn btn-warning btn-sm">Edit</a>
                                </td>
                                @endif
                            </tr>
                            @endforeach
