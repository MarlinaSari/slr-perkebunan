...
@section('title', 'Hasil Prediksi Produksi Kopi')

@section('header', 'Hasil Perhitungan Prediksi Produksi Kopi')

@section('content')
<div class="container">
    <h1>Hasil Prediksi Produksi Kopi</h1>
    ...
    <div class="mb-3">
        <h4>Unduh Data</h4>
        <a href="{{ route('kopi.export.pdf') }}" class="btn btn-danger">Download PDF</a>
        <a href="{{ route('kopi.export.csv') }}" class="btn btn-success">Download CSV</a>
    </div>

    <!-- Tabel untuk Hasil Regresi Linear dan SES, bisa dilihat oleh semua user -->
    <h2>Hasil Regresi Linear</h2>
    @if(!empty($kopiRegression))
                    ...
                    <table class="table table-sm table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if(Auth::user()->role == 'admin')
                                <td>
                                    <form action="{{ route('kopi.destroy', $prediction['periode']) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                    </form>
                                    <a href="{{ route('kopi.edit', $prediction['periode']) }}" class="btn btn-warning btn-sm">Edit</a>
                                </td>
                                @endif
                            </tr>
                            @endforeach
