...
@section('title', 'Hasil Prediksi Produksi Kakao')

@section('header', 'Hasil Perhitungan Prediksi Produksi Kakao')

@section('content')
<div class="container">
    <h1>Hasil Prediksi Produksi Kakao</h1>
    ...
    @if(Auth::user()->role == 'admin')
        ...
        <form action="{{ route('kakao.store') }}" method="POST">
            @csrf
...
<div class="mb-3">
    <h4>Unduh Data</h4>
    <a href="{{ route('kakao.export.pdf') }}" class="btn btn-danger">Download PDF</a>
    <a href="{{ route('kakao.export.csv') }}" class="btn btn-success">Download CSV</a>
</div>

    <!-- Tabel untuk Hasil Regresi Linear dan SES, bisa dilihat oleh semua user -->
    <h2>Hasil Regresi Linear</h2>
    @if(!empty($kakaoRegression))
                    ...
                    <table class="table table-sm table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if(Auth::user()->role == 'admin')
                                <td>
                                    <form action="{{ route('kakao.destroy', $prediction['periode']) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm">Hapus</button>
                                    </form>
                                    <a href="{{ route('kakao.edit', $prediction['periode']) }}" class="btn btn-warning btn-sm">Edit</a>
                                </td>
                                @endif
                            </tr>
                            @endforeach
